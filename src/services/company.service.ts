...
export const createCompany = async (userId: string, companyData: Attributes<Company>) => {
...
export const updateCompanyById = async (id: string, companyData: Attributes<Company>) => {
...
export const deleteCompanyById = async (id: string) => {
  return Company.update({ deleted: false, deleted_at: new Date() }, { where: { id } });
};
...
export function getCompanies(filter: FilterObject, pagination: PaginationOptions) {
